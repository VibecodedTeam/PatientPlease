...
describe('Diagnose', () => {
  it('renders the title and all diagnosis options', () => {
    ...
    expect(screen.getByText('Niewielkie podrażnienie skóry')).toBeInTheDocument();
  ...
  it('disables the submit button while onSubmit is pending, to prevent double submission', async () => {
    ...
    resolveSubmit();
    await waitFor(() => expect(screen.getByText('Prześlij diagnozę')).not.toBeDisabled());
    expect(handleSubmit).toHaveBeenCalledTimes(1);
  });

  it('renders errorMessage when provided', () => {
    render(<Diagnose errorMessage="Nie udało się przesłać diagnozy. Spróbuj ponownie." />);
    expect(
      screen.getByText('Nie udało się przesłać diagnozy. Spróbuj ponownie.'),
    ).toBeInTheDocument();
  });

  it('renders no error message by default', () => {
    render(<Diagnose />);
    expect(screen.queryByText(/nie udało się przesłać/i)).not.toBeInTheDocument();
  });
