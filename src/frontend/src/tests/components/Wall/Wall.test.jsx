...
describe('Wall', () => {
  ...
  it('renders the doctor office wall', async () => {
    ...
    expect(await screen.findByRole('region', { name: /ściana gabinetu lekarskiego/i })).toBeInTheDocument();
  });
  ...
  it('shows an empty shelf message when there are no owned items', async () => {
    ...
    expect(await screen.findByText(/brak zakupionych podręczników/i)).toBeInTheDocument();
  ...
  it('still shows the desk lamp and dermatoscope wall fixtures when there are no owned items', async () => {
    ...
    await screen.findByText(/brak zakupionych podręczników/i);
