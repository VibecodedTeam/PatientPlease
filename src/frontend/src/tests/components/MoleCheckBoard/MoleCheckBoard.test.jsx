...
describe('MoleCheckBoard', () => {
  ...
  it('renders only the collapsed board with the five bold letters, no descriptions or switches', () => {
    ...
    const board = screen.getByRole('button', { name: /otwórz sprawdzian/i });
  ...
  it('opens an expanded dialog into #overlay-root when the board is clicked', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /otwórz sprawdzian/i }));

    const dialog = screen.getByRole('dialog', { name: /zasada abcde/i });
    expect(dialog).toBeInTheDocument();
    expect(document.getElementById('overlay-root').contains(dialog)).toBe(true);
  });

  it('shows all five Polish descriptions together as soon as it expands', async () => {
    const user = userEvent.setup();
    render(<MoleCheckBoard />);
    ...
    expect(screen.getByText('Duża średnica')).toBeInTheDocument();
    expect(screen.getByText('Ewolucja')).toBeInTheDocument();
    expect(screen.getByText(/Średnica większa niż 6 mm/)).toBeInTheDocument();
  });
  ...
  it('calls onSelectionChange with letters in fixed A-E order, not click order', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /otwórz sprawdzian/i }));
    await user.click(screen.getByRole('switch', { name: /oznacz cechę: duża średnica/i }));
    expect(onSelectionChange).toHaveBeenLastCalledWith(['D']);

    await user.click(screen.getByRole('switch', { name: /oznacz cechę: asymetria/i }));
  ...
  it('unchecking a toggle removes it from the emitted selection', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /otwórz sprawdzian/i }));
    const asymetriaSwitch = screen.getByRole('switch', { name: /oznacz cechę: asymetria/i });
  ...
  it('keeps toggles independent of each other (multi-select, not radio)', async () => {
    ...
    expect(screen.getByRole('switch', { name: /oznacz cechę: asymetria/i })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('switch', { name: /oznacz cechę: duża średnica/i })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('switch', { name: /oznacz cechę: brzegi/i })).toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('switch', { name: /oznacz cechę: kolor/i })).toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('switch', { name: /oznacz cechę: ewolucja/i })).toHaveAttribute('aria-checked', 'false');
  });
  ...
  it('pre-checks the letters passed via initialSelected once expanded', async () => {
    ...
    expect(screen.getByRole('switch', { name: /oznacz cechę: brzegi/i })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('switch', { name: /oznacz cechę: ewolucja/i })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('switch', { name: /oznacz cechę: asymetria/i })).toHaveAttribute('aria-checked', 'false');
  });
  ...
  it('keeps selection state across closing and reopening the overlay', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /otwórz sprawdzian/i }));
    await user.click(screen.getByRole('switch', { name: /oznacz cechę: kolor/i }));
    await user.keyboard('{Escape}');
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /otwórz sprawdzian/i }));
    expect(screen.getByRole('switch', { name: /oznacz cechę: kolor/i })).toHaveAttribute('aria-checked', 'true');
  });
