...
`feature/3DHumanRender` (which absorbed and superseded `feature/NightShop`)
introduced a third screen: `StartView`, a skin-cancer-awareness landing
page (ABCDE self-check guide, stats, dermatologist call-to-action) meant to
be reachable on its own, not composed inside `MainView` or `NightView`.
Forcing it into one of those two views as a sub-component would have
misrepresented it as part of the day-phase exam UI, when it's really its
own top-level screen the player can land on independently. The two-view cap
no longer matched what the app actually needed, and the game is expected to
keep adding top-level screens over time (e.g. a results/summary screen, a
settings screen) as it grows.
...
- `src/frontend/src/views/` holds one PascalCase folder per top-level
  ...
  (skin-cancer-awareness landing page, routed at `/start`).
...
- `StartView` is a real view (`views/StartView/`, barrel `index.js`, CSS
  Module, mirrored test in `tests/views/StartView/`) wired to `/start` in
  `App.jsx`, alongside the existing `/` → `MainView` and `/night` →
  `NightView` routes — those two are untouched.
