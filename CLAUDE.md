...
│   │       │   └── Auth/
│   │       ├── views/                  # one folder per top-level screen/route: MainView (day phase), NightView (night/shop phase), StartView (pre-game landing), MinigameView (excision-biopsy minigame) — extend by updating Section 4/6
│   │       │   ├── MainView/
│   │       │   │   ├── index.js
│   │       │   │   ├── MainView.jsx
│   │       │   │   ├── MainView.module.css
...
│   │       │   │   ├── NightView.jsx
│   │       │   │   └── NightView.module.css
│   │       │   └── MinigameView/           # excision-biopsy minigame, routed at /game/main/minigame, wraps components/Excisio
│   │       │       ├── index.js
│   │       │       └── MinigameView.jsx
...
- `src/frontend/src/views/` holds one folder per top-level screen/route wired to its own path in `App.jsx`'s route table — currently `MainView` (day phase), `NightView` (night/shop phase), `StartView` (pre-game skin-cancer-awareness landing page), and `MinigameView` (excision-biopsy minigame, routed at `/game/main/minigame`, a thin wrapper around `components/Excisio`). Adding a new top-level screen means adding its `views/<Name>/` entry here and in Section 6's naming list — it is not a `features/` folder and this list is not meant to stay capped at any fixed number. Every other screen element (patient scene, documents, diagnosis panel, chat, shop, inventory, popups, info board) is a `src/frontend/src/components/` entry composed inside one of those views, not its own view.
...
- Naming conventions:
  - Views: `PascalCase` folder + file, one per top-level route — currently `MainView`, `NightView`, `StartView`, `MinigameView`. This list grows as new top-level screens are added; each addition must be reflected here and in Section 4's rule bullet.
...
- Location: `src/frontend/src/components/OverlayPortal/` (a single shared component, not one per feature).
